...
rf_pred_test_set = np.concatenate([rf_reg_pred,X_test], axis=1)    
...
print('First Dataset RMSE: ', rf_reg_rmse)
print('First Dataset MAE: ', rf_reg_mae)
print('First Dataset R2 Score: ', rf_reg_r2)
...
# Making predictions using the trained model
rf_reg_pred = rf_reg_model.predict(second_X_test)   

rf_reg_pred = rf_reg_pred.reshape(-1,1)
rf_pred_test_set = np.concatenate([rf_reg_pred,second_X_test], axis=1)    
rf_pred_test_set = scaler.inverse_transform(rf_pred_test_set)

# Combine predictions with actual sales
re_list = []
for index in range(0, len(rf_pred_test_set)):
    re_list.append(rf_pred_test_set[index][0] + second_actual_sales[index])

# Merge with the prediction DataFrame
rf_pred_series = pd.Series(re_list, name='rf_pred_2')
predict_df = predict_df.merge(rf_pred_series, left_index=True, right_index=True)

# Evaluate performance metrics
rf_reg_rmse = np.sqrt(mean_squared_error(predict_df['rf_pred_2'], second_month_sales['sales'][-12:]))
rf_reg_mae = mean_absolute_error(predict_df['rf_pred_2'], second_month_sales['sales'][-12:])
rf_reg_r2 = r2_score(predict_df['rf_pred_2'], second_month_sales['sales'][-12:])
print('')
print('Second Dataset RMSE: ', rf_reg_rmse)
print('Second Dataset MAE: ', rf_reg_mae)
print('Second Dataset R2 Score: ', rf_reg_r2)
